...
sc_model.py
------------------------------------------------------
Main set cover algorithm modeling functions and Gurobi optimization code
along with a few additional helper functions.
"""
...
def build_network(F):
    ...
    for i in range(len(F)):
        ...
        for j in range(len(F)):
            if i != j:
                if (
                    F[i].destination == F[j].origin
                    and F[i].arr_min + config.MIN_TRANSFER_TIME <= F[j].dep_min
                ):
                    G.add_edge(i, j)
...
def build_model(F, G):
    ...
    for i in range(len(F)):
        ...
        m.addConstr(
            gurobi.quicksum(x[(u, i)] for u in G.predecessors(i)) == 1,
            name=f"flow_in_{i}"
        )

        # Exactly one arc out of flight i
        m.addConstr(
            gurobi.quicksum(x[(i, v)] for v in G.successors(i)) == 1,
            name=f"flow_out_{i}"
        )

    # Shift Start Initialization Constraints
    for i in range(len(F)):
    ...
    # Shift Start Propagation Constraints
    # Apply only on flight-to-flight compatibility arcs
    for (i, j) in G.edges():
        if i != "s" and j != "t":
            m.addConstr(
                S[j] <= S[i] + H * (1 - x[(i, j)]),
                name=f"sprop_ub_{i}_{j}"
            )
            m.addConstr(
                S[j] >= S[i] - H * (1 - x[(i, j)]),
                name=f"sprop_lb_{i}_{j}"
            )

    # Maximum Shift Duration Constraints
    for i in range(len(F)):
