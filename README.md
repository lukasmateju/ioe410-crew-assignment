...
- **Optional airport-indexed source/sink structure** - The code can replace the single source and sink with airport-indexed source/sink nodes, which changes how routes enter and exit the network.
...
| **Airport-Indexed Source/Sink Structure** *(optional)* | Replace \(s,t\) with airport-indexed nodes \(s_a,t_a\) | When enabled, the code replaces the single source and sink with airport-indexed source and sink nodes. Source arcs and sink arcs are then restricted by flight origin and destination airport. |
...
The airport-indexed source/sink option works identically to the cabin crew model. When enabled (`ENFORCE_SAME_START_END_PILOTS`), each sub-network \(G_q\) uses airport-indexed sources and sinks instead of a single \(s\) and \(t\).
...
The cabin crew and pilot models each assign one crew member per flight. In practice, flights require multiple crew members (a B737 needs 3 cabin crew and 2 pilots). This model extends the previous formulations to handle that requirement, and adds robustness buffers for crew callouts and delays to give us a usable model that assigns individual crew members their own unique routes.
...
- **Airport-indexed sources and sinks** - The single source \(s\) and sink \(t\) can be replaced with a source-sink pair for each airport. Source arcs and sink arcs are then restricted by flight origin and destination airport.
...
| \(N = \{s_a : a \in K\} \cup \hat{F} \cup \{t_a : a \in K\}\) | Full node set when airport-indexed source/sink nodes are enabled: airport source nodes, flight-slot nodes, and airport sink nodes |
...
When airport-indexed sources and sinks are enabled, for each airport \(a \in K\), source node \(s_a\) represents crew members beginning their shift through airport \(a\), and sink node \(t_a\) represents crew members ending their shift through airport \(a\).
...
The same three arc types from the cabin crew and pilot models apply here, adapted for the expanded slot nodes and optional airport-indexed sources/sinks.

| **Arc Type** | **Definition** |
|:--------:|-----------|
| **Source Arc** \(A^{\text{s}} = \{(s_a, (i,k)) : (i,k) \in \hat{F},\ a = b_i\}\) | When airport-indexed source/sink nodes are enabled, connects airport source \(s_a\) to flight-slot \((i,k)\) only when flight \(i\) departs from airport \(a\). |
...
| **Sink Arc** \(A^{\text{e}} = \{((i,k), t_a) : (i,k) \in \hat{F},\ a = e_i\}\) | When airport-indexed source/sink nodes are enabled, connects flight-slot \((i,k)\) to airport sink \(t_a\) only when flight \(i\) arrives at airport \(a\). |
...
| Variable | Domain | Definition |
|----------|--------|-----------|
| \(x_{ij}\) | \(\{0,1\}\), \(\forall (i,j) \in A\) | **Arc Selection Variable:** Equals 1 if a crew member traverses arc \((i,j)\) as part of their route, 0 otherwise. Arcs now connect flight-slot nodes and, when enabled, airport-indexed sources/sinks. |
...
| Function | Definition |
|----------|--------|
| \(\min \sum_{a \in K} \sum_{(i,k) \in \hat{F}} x_{s_a,(i,k)}\) | When airport-indexed source/sink nodes are enabled, minimize the total number of crew members across all airports (total source arcs used) |
...
## Route Reconstruction (Post-Processing)

After the optimization model has been solved, the code reconstructs crew routes by tracing selected arcs from source nodes through flight-slot nodes until a sink node is reached.

For each recovered route, the code also records the shift interval:

- **Clock-in time** = the shift start variable value of the first slot in the route
- **Clock-out time** = the arrival time of the last flight in the route

This is used for reporting crew routes and shift times in the output.
